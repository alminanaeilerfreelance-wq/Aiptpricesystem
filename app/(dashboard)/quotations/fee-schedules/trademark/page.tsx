...
interface RuleMap {
  // key: `${countryId}_${procedureId}` -> PricingRule
  [key: string]: PricingRule;
}

export default function TrademarkFeeSchedulePage() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        const procs = procData.procedures;
        const countries = countryData.countries;
        const rules = rulesData.pricingRules;

        // Build a lookup: procedure name (lowercase) + country name (lowercase) -> rule
        const map: RuleMap = {};
        rules.forEach((rule) => {
  ...
  if (loading) {
    return (
      <div className="flex flex-col h-full">
        <Topbar
          title="Trademark Fee Schedule"
          breadcrumbs={[{ label: 'Fee Schedules' }, { label: 'Trademark' }]}
        />
        ...
        {procedures.length === 0 ? (
          <Card>
            <p className="text-sm text-gray-500 text-center py-6">
              No Trademark procedures found. Add procedures first.
            </p>
          </Card>
        ) : (
          <Card padding="p-0">
            <div className="overflow-x-auto">
              <table className="border-collapse text-sm" style={{ minWidth: `${procedures.length * 220 + 200}px` }}>
                <thead className="bg-surface">
                  {/* Header Row 1: Procedure names spanning 3 sub-cols each */}
                  <tr>
                    {/* Empty cells for Flag + Country columns */}
                    <th
                    ...
                    {procedures.map((proc) => (
                      <th
                        key={proc._id}
                        colSpan={3}
                        className="table-header text-center border-l border-border bg-blue-50 text-blue-800"
                      >
                        {proc.name}
                      </th>
                    ))}
                  ...
                  {/* Header Row 2: Sub-columns for each procedure */}
                  <tr>
                    {procedures.map((proc) => (
